...
<template>
  <section ref="sectionRef" class="py-24 md:py-32 bg-gray-50 relative overflow-hidden">
    ...
    <div class="container mx-auto px-6 max-w-4xl relative z-10">
      ...
      >
        ...
        <span class="inline-block mt-3 text-sm text-gray-400 bg-gray-100 px-3 py-1 rounded-full">{{ faqs.length }} Questions</span>
      </div>
      ...
      <div class="space-y-4">
        <div
          ...
          :class="[
            ...
            faq.isOpen ? 'ring-1 ring-primary/20 shadow-md' : ''
          ]"
          :style="{ transitionDelay: isVisible ? `${0.1 * index}s` : '0s' }"
        >
          <button
            @click="toggleFaq(index)"
            class="w-full px-6 py-5 text-left flex justify-between items-center transition-colors group"
          >
            <span class="font-semibold text-dark-slate pr-4 group-hover:text-primary transition-colors duration-300">{{ faq.question }}</span>
            <div
              class="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 transition-all duration-300"
              :class="faq.isOpen ? 'bg-primary text-white rotate-180' : 'bg-gray-100 text-gray-400 group-hover:bg-primary/10 group-hover:text-primary'"
            >
              <svg
          ...
          >
            <div class="pb-5 text-gray-500 leading-relaxed">
              {{ faq.answer }}
            </div>
          </div>
        </div>
